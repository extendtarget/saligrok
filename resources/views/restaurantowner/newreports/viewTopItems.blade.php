...
<div class="page-header">
    <div class="page-header-content header-elements-md-inline">
        ...
        <div class="header-elements">
            <form action="{{ route('restaurant.viewTopItems') }}" method="GET">
...
<div class="content mb-5">
    <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>    
                                    <a href="{{ route('restaurant.get.editItem', $item->item_id) }}" class="letter-icon-title">{{ $item->name }}</a>
                                </td>
                                <td>
                                    {{ $item->qty}}
                                </td>
                                <td>
                                    <span class="text-semibold no-margin">{{ config('setting.currencyFormat') }} {{ round($item->price * $item->qty) }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>    
                                    <a href="{{ route('restaurant.get.editItem', $top_item->item_id) }}" class="letter-icon-title">{{ $top_item->name }}</a>
                                </td>
                                <td>
                                    {{ $top_item->qty}}
                                </td>
                                <td>
                                    <span class="text-semibold no-margin">{{ config('setting.currencyFormat') }} {{ round($top_item->price * $top_item->qty) }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="col-xl-6">
            <div class="panel panel-flat dashboard-main-col mt-4" style="min-height: 30rem;">
                <div class="panel-heading">
                    <h4 class="panel-title pl-3 pt-3"><strong>Top 10 Most Sold Item Addons </strong></h4>
                    <hr>
                </div>
                <div class="table-responsive">
                    <table class="table text-nowrap">
                        @if($top_item_addons_total)
                            <thead>
                                <tr>
                                    <th>Item Name</th>
                                    {{-- <th>Price</th> --}}
                            ...
                            <tbody>
                                @foreach($top_item_addons_total as $addon_item)
                                <tr>
                                    <td>    
                                        <a href="{{ route('restaurant.editAddon', $addon_item->id) }}" class="letter-icon-title">{{ $addon_item->addon_name }}</a>
                                    </td>
                                    {{-- <td>
                                        <span class="text-semibold no-margin">{{ config('setting.currencyFormat') }} {{ round($addon_item->addon_price) }}</span>
                                    </td> --}}
                                </tr>
                                @endforeach
                            </tbody>
