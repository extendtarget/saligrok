                        ...
                        <tr>
                            <td>
                                ...
                                <img src="{{substr(url("/"), 0, strrpos(url("/"), '/'))}}{{ $item->image }}"
                                    alt="{{ $item->name ?? 'غير متوفر' }}" height="80" width="80" style="border-radius: 0.275rem;">
                                @else
                                <span>{{__('storeDashboard.noImageAvailable')}}</span>
                                @endif
                            </td>
                            <td>{{ $item->name ?? 'غير متوفر' }}</td>
                            <td>{{ $item->restaurant ? $item->restaurant->name : 'غير متوفر' }}</td>
                            <td>{{ $item->item_category ? $item->item_category->name : 'غير متوفر' }}</td>
                            <td>{{ $item->price ? config('setting.currencyFormat') . $item->price : 'غير متوفر' }}</td>
                            <td>
                            ...
                            <td>{{ $item->created_at ? $item->created_at->diffForHumans() : 'غير متوفر' }}</td>
                            <td class="text-center">
                                <div class="btn-group btn-group-justified align-items-center">
                                    <a href="{{ route('restaurant.get.editItem', $item->id) }}"
                                        class="btn btn-sm btn-primary"> {{__('storeDashboard.edit')}}
                                    </a>
                                    ...
                                    <form action="{{ route('restaurant.deleteItem', $item->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger">{{__('storeDashboard.delete')}}</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
    ...
    <div class="card">
        <div class="card-header pb-0">
            <div class="d-flex">
                ...
                <div class="ml-3">
                    <h4 class="mb-0"><strong>{{ $item->name ?? 'غير متوفر' }}</strong></h4>
                    <span>{{ $item->restaurant ? $item->restaurant->name : 'غير متوفر' }}</span><br>
                    <span>{{ $item->item_category ? $item->item_category->name : 'غير متوفر' }}</span>
                </div>
            </div>
        </div>
        <hr>
        <div class="card-body pt-0 pb-2">
            <div class="d-flex justify-content-between">
                <div>
                    <h4 class="mb-0"><strong>{{ $item->price ? config('setting.currencyFormat') . $item->price : 'غير متوفر' }}</strong></h4>
                </div>
                <div>
                    @if($item->is_recommended)
                    <span class="badge badge-flat border-grey-800 text-danger text-capitalize mr-1">
