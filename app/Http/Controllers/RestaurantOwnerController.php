...
class RestaurantOwnerController extends Controller
{
    ...
    public function acceptOrder($id, Request $request)
    {
        ...
        if ($order->orderstatus_id == '1') {
            if (isset($request->prep_time)) {
                $order->prep_time = Carbon::now()->addMinutes($request->prep_time)->toDateTimeString();
            }
    ...
    public function markSelfPickupOrderAsCompleted($id)
    {
        ...
        $order = Order::where('id', $id)->whereIn('restaurant_id', $restaurantIds)->with(['restaurant', 'user'])->first();
        ...
        public function updateRestaurant(Request $request)
    {
        ...
        if (!$restaurant) {
            Log::channel('update-order')->error('Restaurant not found or access denied for ID: ' . $request->id);
        ...
        try {
            ...
            $scheduleData = $request->except(['_token', 'restaurant_id', 'name', 'description', 'image', 'old_image', 'delivery_time', 'price_range', 'is_pureveg', 'certificate', 'address', 'pincode', 'landmark', 'latitude', 'longitude', 'restaurant_charges', 'min_order_price', 'delivery_type', 'is_schedulable', 'accept_scheduled_orders', 'schedule_slot_buffer', 'store_payment_gateways']);
            if (!empty($scheduleData)) {
                ...
                foreach ($scheduleData as $day => $times) {
                    if (!is_array($times) || empty($times)) {
                        $str .= '"' . $day . '":[]';
                        Log::channel('update-order')->info("No schedule provided for $day");
                    } else {
                        $str .= '"' . $day . '":[';
                        $openTimes = [];
                        $closeTimes = [];
                        ...
                        foreach ($openTimes as $index => $openTime) {
                            if (empty($openTime) || empty($closeTimes[$index])) {
                                Log::channel('update-order')->error('Invalid schedule: Missing open or close time', [
                                    'day' => $day,
                                    'open' => $openTime,
                                    'close' => $closeTimes[$index]
                                ]);
                                return redirect()->back()->with(['message' => "Missing open or close time for $day"]);
                            }
    
                            try {
                                $open = Carbon::createFromFormat('H:i', $openTime, 'UTC');
                                $close = Carbon::createFromFormat('H:i', $closeTimes[$index], 'UTC');
    
                                if (!$open || !$close) {
                                    Log::channel('update-order')->error('Invalid time format', [
                                        'day' => $day,
                                        'open' => $openTime,
                                        'close' => $closeTimes[$index]
                                    ]);
                                    return redirect()->back()->with(['message' => "Invalid time format for $day"]);
                                }
    
                                // Handle midnight-crossing schedules
                                if ($close->lessThan($open)) {
                                    $close->addDay();
                                    Log::channel('update-order')->info('Midnight-crossing schedule detected', [
                                        'day' => $day,
                                        'open' => $openTime,
                                        'close' => $closeTimes[$index]
                                    ]);
                                ...
                                if ($open->greaterThanOrEqualTo($close)) {
                                    Log::channel('update-order')->error('Invalid schedule: Opening time is not before closing time', [
                                        'day' => $day,
                                        'open' => $openTime,
                                        'close' => $closeTimes[$index]
                                    ]);
                                    return redirect()->back()->with(['message' => "Opening and Closing time is incorrect for $day"]);
                                }
                            } catch (\Exception $e) {
                                Log::channel('update-order')->error('Time parsing error', [
                                    'day' => $day,
                                    'open' => $openTime,
                                    'close' => $closeTimes[$index],
                                    'error' => $e->getMessage()
                                ]);
                                return redirect()->back()->with(['message' => "Invalid time format for $day: " . $e->getMessage()]);
                            }
                        }
                        $str .= ']';
                    }
    
                    if ($i != count($scheduleData) - 1) {
                        $str .= ',';
                    }
                    $i++;
                }
                ...
                Log::channel('update-order')->info('Schedule data prepared: ' . $str);
            }
    
            // Update payment gateways
            $restaurant->payment_gateways()->sync($request->input('store_payment_gateways', []));
            Log::channel('update-order')->info('Payment gateways synced: ' . json_encode($request->input('store_payment_gateways', [])));
  ...
  // Extend By Aya
    public function deleteItemCategory($id) {
        ...
        if (!$itemCategory) {
            return redirect()->back()->with(['message' => 'Item Category Not Found']);
        }

        if ($itemCategory->user_id != Auth::user()->id) {
        ...
        try {
            ...
            $itemCategory->delete();
            return redirect()->back()->with(['success' => 'Item Category Deleted']);
        } catch (\Throwable $th) {
...
// aya end ---------------------------------
    ...
    public function deleteItem($id) {
        ...
        if (!$item || !in_array($item->restaurant_id, $restaurantIds)) {
            return redirect()->back()->with(['message' => 'Item Not Found or Unauthorized']);
        }
...
public function deleteAddonCategory($id) {
    ...
    if (!$addonCategory) {
        return redirect()->route('restaurant.addonCategories')->with(['message' => "Addon Category Not Found"]);
    }

    if ($addonCategory->user_id != $user->id) {
        return redirect()->route('restaurant.addonCategories')->with(['message' => "Unauthorized"]);
    }

    try {
        $addonCategory->addons()->delete();
        $addonCategory->delete();
        return redirect()->route('restaurant.addonCategories')->with(['success' => "Addon Category Deleted"]);
    } catch (\Throwable $th) {
        return redirect()->route('restaurant.addonCategories')->with(['message' => $th->getMessage()]);
    }
}
// ---------------------------------------------
    /**
