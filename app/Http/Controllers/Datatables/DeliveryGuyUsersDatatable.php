        ...
        return Datatables::of($users)
           ...
           ->addColumn('action', function ($user) {
                return '<div class="btn-group btn-group-justified">' .
                    '<a href="' . route('admin.get.manageDeliveryGuysRestaurants', $user->id) . '" class="btn btn-sm btn-secondary mr-2"> Manage Delivery Stores</a>' .
                    '<a href="' . route('admin.get.editUser', $user->id) . '" class="btn btn-sm btn-primary mr-2"> View</a>' .
                    '<a href="' . route('admin.deleteUser', $user->id) . '" class="btn btn-sm btn-danger deleteUserBtn" data-userid="' . $user->id . '" onclick="return confirm(\'Confirm delete?\')"> Delete</a>' .
                    '</div>';
            })
