    ...
    public function saveSettings(Request $request, Factory $cache)
    {
        try {
            ...
            if (isset($request->order_count)) {
                $incentives = [];
                for ($i = 0; $i < count($request->order_count); $i++) {
                    $incentives[] = [
                        'order_count' => $request->order_count[$i],
                        'incentive_amount' => $request->incentive_amount[$i],
                    ];
                }
                $deliveryGuyIncentiveData = json_encode($incentives);

                $deliveryGuyIncentive = Setting::where('key', 'deliveryGuyIncentiveData')->first();
                if (!$deliveryGuyIncentive) {
                    $deliveryGuyIncentive = new Setting();
                    $deliveryGuyIncentive->key = "deliveryGuyIncentiveData";
                }
                $deliveryGuyIncentive->value = $deliveryGuyIncentiveData;
                $deliveryGuyIncentive->save();
            }
            ...
            $checkboxesSettings = ['autoAssignNearestDeliveryGuy', 'enableReferAndEarn', 'enableDeliveryGuyIncentive', 'showPromoSlider', 'showMap', 'enablePushNotification', 'enablePushNotificationOrders', 'showGdpr', 'enableGoogleAnalytics', 'taxApplicable', 'enSOV', 'enSPU', 'enableFacebookLogin', 'enableGoogleLogin', 'enableDeliveryPin', 'hidePriceWhenZero', 'enableDeliveryGuyEarning', 'enPassResetEmail', 'showPercentageDiscount', 'showVegNonVegBadge', 'showFromNowDate', 'enDelChrRnd', 'expandAllItemMenu', 'smsRestaurantNotify', 'smsRestOrderValue', 'smsDeliveryNotify', 'smsOrderNotify', 'showOrderAddonsDelivery', 'showDeliveryFullAddressOnList', 'showUserInfoBeforePickup', 'recommendedLayoutV2', 'flatApartmentAddressRequired', 'showInActiveItemsToo', 'enGDMA', 'showPriceAndOrderCommentsDelivery', 'useSimpleSpinner', 'randomizeStores', 'showCouponDescriptionOnSuccess', 'stripeAcceptAliPay', 'stripeAcceptBitCoin', 'enIOSPWAPopup', 'mockSearchOnHomepage', 'stripeInlineCardCheckout', 'stripeAcceptIdealPayment', 'stripeAcceptFpxPayment', 'stripeCheckoutPostalCode', 'googleFullAddress', 'showDeliveryCollection', 'allowPaymentGatewaySelection', 'sortSelfpickupStoresByDistance', 'countQuantityAsTotalItemsOnCart', 'allowPaymentGatewaySelection', 'showTipsPercentage', 'showTipsAmount', 'showTryItOnPhone', 'showAddonPricesOnCart', 'showCustomerVatNumber', 'showPwaInstallPromptFooter', 'showCashChange', 'sortDeliveryStoresByDistance', 'enCODonSF', 'sendOrderInvoiceOverEmail', 'hideCustomerDetailsFromStoreOwner', 'iHaveFoodomaaAndroidApp', 'iHaveFoodomaaDeliveryApp', 'iHaveFoodomaaStoreApp', 'hasSocketPush'];
            ...
            foreach ($checkboxesSettings as $checkboxSetting) {
                ...
                if ($setting) {
                    ...
                    DB::table('settings')->where('key', $checkboxSetting)->update(['value' => $value]);
                } else {
                    if ($checkboxSetting != null || $checkboxSetting != '') {
                        ...
                        DB::table('settings')->insert(['key' => $checkboxSetting, 'value' => $value]);
                    }
                }
            }
